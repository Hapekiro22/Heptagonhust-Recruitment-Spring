...
### v0.7.6
    ...
    记录一下当前所需最小的同步点：在完成C的传输后一定要分流同步


## v0.8
现在最大的困难是传输困难，在这种情况下，只要传输的数据量不变，传输就难以优化
所以，最直接的解决办法就是不要进行大量传输，而是在一开始的时候就进入gpu计算，从源头上解决问题
开始的数据量是变换后数据量的1%，只要gpu中可以保持相同的性能，那么问题可以得到很好的解决

尝试1：
    transform分块处理：效率改变不大，核心原因是我没有规划cpu寄存器分配
...
目前看来，想要实现流水线，io优化仍然是重中之重，所以，优先处理压缩问题
2.没有库，手写函数太难了
1.折中方案：对输出进行增量检测，但问题是契合度仍然不知道高不高
...
今日规划：
    1h：尝试低精度
    0.5h：设置延迟：上一个DTOH开始后才会进行下一个HTOD；这样就不会有多个重叠
    13：00-14；30：完成ppt
    15：00-20：00 尝试流水线
